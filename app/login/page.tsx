...
export default function LoginPage() {
  ...
  async function signIn() {
    ...
    try {
      ...
      // Persist learner identity for the iframe portal (used for webhook payloads)
      try {
        localStorage.setItem("qs_user_email", emailClean);
        localStorage.setItem("qs_user_id", userRes.user.id);
        localStorage.setItem("qs_company_slug", company.slug);
        const fullName = (profile as any)?.full_name || "";
        if (fullName) localStorage.setItem("qs_user_name", fullName);
      } catch {
